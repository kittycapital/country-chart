...
<body>
    <div class="container">
        <div class="header">
            <h1>글로벌 국가별 ETF 퍼포먼스</h1>
            <div class="sub">주요 국가 ETF 수익률 비교 — 글로벌 자금 흐름 한눈에 파악</div>
        ...
        <div class="footer">데이터 출처: Yahoo Finance · 국가별 대표 ETF 기준 · 투자 판단은 본인의 책임입니다</div>
    </div>
    ...
    <script>
        /* ====== SHARE ====== */
        const SHARE_URL = 'https://herdvibe.com/country-etf';
        const SHARE_TITLE = '글로벌 국가별 ETF 퍼포먼스 — Herdvibe';
        const SHARE_DESC = '주요 국가 ETF 수익률 비교 | Herdvibe';
        ...
        function shareKakao() {{
            ...
            if (window.Kakao) {{
                Kakao.Share.sendDefault({{
                    objectType: 'feed',
                    content: {{ title: SHARE_TITLE, description: SHARE_DESC, imageUrl: 'https://herdvibe.com/og-country.png', link: {{ mobileWebUrl: SHARE_URL, webUrl: SHARE_URL }} }}
                }});
            }}
        }}
        ...
        function createLegend() {{
            ...
            legend.innerHTML = Object.entries(ASSETS_DATA).map(([symbol, data]) =>
                `<div class="legend-item" data-symbol="${{symbol}}">
                    ...
                    <span class="legend-label">${{symbol}} (${{data.name}})</span>
                </div>`
            ).join('');
